...
import { X, CheckCircle, UserPlus, Search, User } from 'lucide-react';
...
const inputClass = "w-full px-3 py-2 text-[13px] border border-gray-200 rounded-lg bg-white text-gray-800 placeholder:text-gray-300 focus:outline-none focus:ring-1 focus:ring-indigo-500";
...
export const AddMemberForm = ({ onClose }) => {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl border border-gray-200/80 shadow-[0_16px_48px_rgba(0,0,0,0.14)] w-full max-w-md mx-4"
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-5 space-y-4">
          <Field label="Search User" error={errors.query?.message}>
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 text-[13px] border border-gray-200 rounded-lg bg-white text-gray-800 placeholder:text-gray-300 focus:outline-none focus:ring-1 focus:ring-indigo-500"
              />
            </div>
              ...
              <div className="mt-1 bg-white border border-gray-200 rounded-lg shadow-[0_4px_12px_rgba(0,0,0,0.08)] overflow-hidden max-h-40 overflow-y-auto">
                {userResults.map((user) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 px-3 py-2.5 text-left hover:bg-gray-50 transition-colors text-[13px] ${selectedUser?._id === user._id ? 'bg-indigo-50' : ''}`}
                  >
                    <div className="h-6 w-6 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 text-[10px] font-semibold shrink-0">
          ...
          {selectedUser && (
            <div className="flex items-center gap-2.5 px-3 py-2.5 bg-indigo-50 border border-indigo-100 rounded-lg">
              ...
              <button type="button" onClick={() => setSelectedUser(null)} className="text-indigo-400 hover:text-indigo-600 transition-colors">
                <X size={13} />
              </button>
            </div>
          )}
          ...
          <div className="flex gap-2 pt-1">
            <button
              type="submit"
              disabled={isLoading}
              className="flex-1 flex items-center justify-center gap-1.5 py-2.5 bg-indigo-600 text-white text-[13px] font-semibold rounded-lg hover:bg-indigo-700 transition-colors disabled:opacity-60"
            >
              <UserPlus size={14} />
